...
export default function Groups(props: { onExit: () => void, onEnter: () => void }) {
    ...
    useNuiEvent('groups:addGroupStage', async (stage: { isDone: boolean, name: string, id: number }[]) => {
        setGroupStage(stage);
    });
    useNuiEvent('groups:addStatusPage', async (stage: { isDone: boolean, name: string, id: number }[]) => {
        setGroupStage(stage);
    });
            ...
            >
                <Transition
                    ...
                    onEnter={async () => {
                        ...
                        const newRes = await fetchNui('getStatusPage', "Ok");
                        const parsedData = JSON.parse(res as string);
                        setGroupData(parsedData.data);
                        setPlayerSource(parsedData.src);
                    }}
                >
                    {(styles) => <div style={{
                        ...styles,
                        ...
                        <div style={{
                            ...
                            <svg onClick={() => {
                                ...
                                <path d="M7.55555 12H12M12 12H16.4444M12 12V16.4444M12 12V7.55555M12 22C6.47716 22 2 17.5229 2 12C2 6.47716 6.47716 2 12 2C17.5229 2 22 6.47716 22 12C22 17.5229 17.5229 22 12 22Z" stroke="#0A84FF" stroke-width="2.5" strokeLinecap="round" stroke-linejoin="round" />
                            </svg>
                        </div>
                        ...
                        <div style={{
                            ...
                            {groupData && Object.values(groupData).map((group, i) => {
                                return (
                                    <div key={i} style={{
                                        width: '100%',
                                        height: '3.5vw',
                                        ...
                                        <div style={{
                                            ...
                                            <div style={{ display: 'flex', fontSize: '0.8vw' }}>{group.GName}</div>
                                            <div style={{ display: 'flex', alignItems: 'center' }}>
                                                <div style={{ display: 'flex', color: 'rgba(255,255,255, 0.5)', fontSize: '0.8vw', alignItems: 'center', gap: '0.2vw', lineHeight: '0.5vw' }}>
                                                    <svg width="0.8333333333333334vw" height="0.8333333333333334vw" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        <div style={{
                                            ...
                                            {group.leader === playerSource ? <svg onClick={() => {
                                            ...
                                            }} className='clickanimation' width="0.78125vw" height="0.78125vw" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                ...
                                                group.members.find(member => member.Player === playerSource) ?
                                                    <svg onClick={() => {
                                                    ...
                                                    : group.status === "WAITING" ? <svg onClick={() => {
                                                        ...
                                                        setInputShow(true);
                                                        setSelectedGroupId(group.id);
                                                        setSelectedPassword(group.GPass);
                                                    }} className='clickanimation' width="1.0416666666666667vw" height="1.0416666666666667vw" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <InputDialog show={inputShow} placeholder={inputPlaceholder} description={inputDescription} title={inputTitle} onConfirm={async (e: string) => {
                    ...
                    if (inputTitle === 'Create Group') {
                        ...
                        if (e.toLowerCase() === 'yes') {
                            const res = await fetchNui('jobcenter_leave_grouped', { id: selectedgroupId });
                        }
                    } else if (inputTitle === 'Join Group') {
                        if (e === selectedPassword) {
                            const res = await fetchNui('jobcenter_JoinTheGroup', { id: selectedgroupId });
                        }
                    }
                }} onCancel={() => {
                    setInputShow(false);
